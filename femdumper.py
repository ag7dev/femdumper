...
os.makedirs(femdumper_folder, exist_ok=True) if not os.path.exists(femdumper_folder) else shutil.rmtree(femdumper_folder) or os.makedirs(femdumper_folder)
WebhookFilePath = os.path.join(femdumper_folder, "discord_webhooks.txt")
VarFilePath = os.path.join(femdumper_folder, "variables.txt")
tiggerFilePath = os.path.join(femdumper_folder, "trigger_events.txt")
ac_keywords_file_path = os.path.join(femdumper_folder, "anticheat_keywords.txt")
acs_founds_file_path = os.path.join(femdumper_folder, "acs_founds.txt")
AnticheatKeywords = ["Anticheat", "Godmode", "Noclip", "Eulen", "Detection", "Shield", "Fiveguard", "Noclip", "deltax", "waveshield", "spaceshield", "mixas", "protected", "cheater", "cheat", "banNoclip", "Detects", "blacklisted", "CHEATER BANNED:", "core_shield", "freecam"]
...
def find_discord_webhooks(path, output_file):
    ...
    def is_webhook_valid(url):
        try:
            ...
            # Valid webhook if we get status code 200
    ...
    # Save valid webhooks to the output file
    with open(output_file, "w", encoding="utf-8") as output:
...
def delete_webhook(webhook_url):
    """
    Deletes a webhook. An Idea by Fannzy
    """
    try:
        response = requests.delete(webhook_url)
        if response.status_code == 204:
            print(Fore.GREEN + f"Webhook successfully deleted: {webhook_url}" + Fore.RESET)
        else:
            print(Fore.RED + f"Error deleting webhook {webhook_url}: {response.status_code}" + Fore.RESET)
    except requests.RequestException as e:
        print(Fore.RED + f"Error deleting webhook {webhook_url}: {e}" + Fore.RESET)
    time.sleep(3)


def load_and_delete_webhooks(webhook_file_path):
    """
    Loads and deletes webhooks from the given file.
    """
    try:
        with open(webhook_file_path, "r", encoding="utf-8") as file:
            lines = file.readlines()
            # Skip the header line and extract webhook URLs
            webhooks = []
            for line in lines[1:]:  # Start from 1 to skip the header
                parts = line.split('|')
                if len(parts) > 1:  # Check if there are enough parts
                    webhooks.append(parts[1].strip())
                else:
                    print(Fore.RED + "Warning: Skipping line due to unexpected format: " + line + Fore.RESET)

        # Now delete each webhook
        for webhook_url in webhooks:
            delete_webhook(webhook_url)

    except Exception as e:
        print(Fore.RED + f"Error loading webhooks from file: {e}" + Fore.RESET)

def get_webhook_info(webhook_url=None):
    """
    Retrieves information about a webhook, including creator and other details.
    If no webhook URL is provided, it loads from a file.
    """
    print(Fore.CYAN + "-------------------------------------------------------------------------------" + Fore.RESET)

    if webhook_url is None:
        # Load webhooks from the file
        try:
            with open(WebhookFilePath, "r", encoding="utf-8") as file:
                lines = file.readlines()
                # Skip the header line and extract webhook URLs
                webhook_urls = []
                for line in lines[1:]:
                    parts = line.split('|')
                    if len(parts) > 1:  # Check if there are enough parts
                        webhook_urls.append(parts[1].strip())
                    else:
                        print(Fore.RED + "Warning: Skipping line due to unexpected format: " + line + Fore.RESET)
        except Exception as e:
            print(Fore.RED + f"Error loading webhooks from file: {e}" + Fore.RESET)
        ...
        # Iterate over each webhook URL and retrieve its info
        for webhook_url in webhook_urls:
            print(Fore.CYAN + f"Webhook information for: {webhook_url}" + Fore.RESET)
            try:
                response = requests.get(webhook_url)
                if response.status_code == 200:
                    webhook_data = response.json()
                    print(Fore.GREEN + "Webhook information:" + Fore.RESET)
                    print(Fore.GREEN + json.dumps(webhook_data, indent=4) + Fore.RESET)
                else:
                    print(Fore.RED + f"Error retrieving webhook information: {response.status_code}" + Fore.RESET)
                    time.sleep(3)
            except requests.RequestException as e:
                print(Fore.RED + f"Error retrieving webhook information: {e}" + Fore.RESET)
                time.sleep(3)
            print(Fore.CYAN + "-------------------------------------------------------------------------------" + Fore.RESET)

            # Wait for user input before proceeding to the next webhook
            input(Fore.YELLOW + "Press Enter to continue to the next webhook..." + Fore.RESET)
...
while True:
    ...
    print(Fore.WHITE + "7. Delete Webhooks")  
    print(Fore.WHITE + "8. Get Webhook Informations")  
    ...
    if choice == "1":
        ...
        clear_screen()
        print(Fore.YELLOW + "-------------------------------------------------------------------------------" + Fore.RESET)
        type_writer_animation(Fore.BLUE + "Loading and deleting webhooks...")
        print(Fore.YELLOW + "-------------------------------------------------------------------------------" + Fore.RESET)
        load_and_delete_webhooks(WebhookFilePath)
    elif choice == "8":
        clear_screen()
        print(Fore.YELLOW + "-------------------------------------------------------------------------------" + Fore.RESET)
        type_writer_animation(Fore.BLUE + "Loading Webhook Info...")
        print(Fore.YELLOW + "-------------------------------------------------------------------------------" + Fore.RESET)
        get_webhook_info()
    elif choice == "9":
